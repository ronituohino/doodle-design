...
const ParcelAddressSelection = ({ formik, setAddress }) => {
  ...
  const foundDeliveryPoints = deliveryPoints && deliveryPoints.locations;
  const notFoundDeliveryPoints = deliveryPoints && !deliveryPoints.locations;

  const fetchDeliveryPoints = async () => {
    const response = await getPostalPoints(formik.values.searchZipCode, 5);
    ...
    <Paper variant="outlined" sx={{ padding: 2, marginRight: 6 }}>
      ...
      {foundDeliveryPoints &&
        deliveryPoints.locations.map(
          p =>
            appropriatePoint(p) && (
              <ParcelAddress key={p.id} point={p} selectCallback={transform} />
            )
        )}
